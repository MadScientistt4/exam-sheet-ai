...
**Reliability**: the free Gemini tier occasionally rate-limits, returns a transient 5xx, or (as actually happened once — `gemini-2.0-flash` was retired outright, returning a 404) stops supporting a specific dated model entirely. `src/lib/gemini.ts` handles this with a model fallback chain (configured `GEMINI_MODEL` → `gemini-flash-latest` → `gemini-2.5-flash` → `gemini-2.5-flash-lite`, deduped) — a 500/503 gets one immediate retry on the same model, anything else (429, 404, bad JSON, etc.) moves straight to the next model in the chain. `gemini-flash-latest` is Google's rolling alias for their current recommended flash model, so the default path shouldn't go stale the same way again; the pinned models behind it are a known-good backstop. Both extraction calls go through this same wrapper.
