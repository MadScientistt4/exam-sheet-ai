...
**Reliability**: the free Gemini tier occasionally rate-limits, returns a transient 5xx, or (as actually happened once — `gemini-2.0-flash` was retired outright, returning a 404) stops supporting a specific dated model entirely. `src/lib/gemini.ts` handles this with a model fallback chain (configured `GEMINI_MODEL` → `gemini-flash-latest` → `gemini-2.5-flash` → `gemini-2.5-flash-lite`, deduped) — any failure moves straight to the next model, one try each, no same-model retry. `gemini-flash-latest` is Google's rolling alias for their current recommended flash model, so the default path shouldn't go stale the same way again; the pinned models behind it are a known-good backstop. Both extraction calls go through this same wrapper.

**Function timeout**: `/api/map-answers` is the heavy call (full answer sheet + region detection + grading + overall feedback, potentially multi-page) and both routes set `maxDuration = 60` — the max Vercel's Hobby plan allows without Fluid Compute. The same-model-retry removal above exists specifically to keep worst-case latency (now up to 3 sequential model attempts instead of up to 6) under that ceiling. If a single Gemini call is itself slow enough to exceed 60s on a large real scan, the next lever is enabling Fluid Compute in the Vercel project settings (raises the ceiling to 300s) — that's a dashboard change, not code.
