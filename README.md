...
2. **Answer mapping + grading** (`POST /api/map-answers`) — the answer sheet plus the extracted question list (including any MCQ options) are sent to Gemini in one call. For each question it returns whether it was attempted, one bounding-box region per contiguous block of handwriting (as `0-1` fractions of that page, supporting answers that span multiple regions/pages), a score, and one-sentence feedback. For MCQs, a bare selected letter (e.g. "D") is resolved against that question's option text so it's graded for correctness, not just presence. Handwritten content that doesn't correspond to any question comes back as a separate `unmatched` list. The same call also returns a 2-3 sentence `overallFeedback` — a holistic summary of the student's performance across the whole exam — so there's no extra round-trip for it.
   ...
   **Multi-page answers**: an answer's `regions` array can span any number of pages (verified on a real scanned notebook where an answer ran across a page break). Selecting a question jumps to the page its answer starts on; if it continues onto other pages, the answer sheet panel shows a "this answer spans multiple pages" chip row so the rest is easy to find instead of silently hidden on a page there'd be no reason to visit.

The mapping screen shows a dedicated grading summary (score, correct/partial/incorrect/unanswered breakdown, and the overall feedback) above the question list and answer sheet panels, rather than a single-line score subtitle. The total-marks denominator sums every question's `maxMarks`, answered or not — an unanswered question pulls the score down instead of silently shrinking the denominator (a real bug: a 3-question, 6-mark exam with one question skipped was showing "3/4 (75%)" instead of the correct "3/6 (50%)").
...
Answer-sheet pages are rendered client-side from the uploaded file via `pdfjs-dist` (canvas), and the highlight regions are drawn as absolutely-positioned overlays scaled as percentages, so they track zoom automatically without needing pixel math. The model occasionally underestimates a line's trailing edge and clips the last word or two — since a line of handwriting on an answer sheet only ever belongs to one answer, the overlay pads the right edge outward by a small margin (clamped to the page boundary) as a safety net, alongside a prompt change that asks the model to err on the side of the full line width in the first place.

A trickier failure mode showed up on real dense handwriting: when consecutive answers are written close together (especially where a student re-writes the question, sometimes with their own numbering, right before each answer), the model would sometimes get the *bottom* edge of a region wrong — including the tail of the previous answer while cutting off the last lines of the correct one. A prompt-only fix (asking it to anchor each region to its true start/end) helped but wasn't reliable enough on its own, since it still depended on the model's own judgment of where an answer ends — exactly the dimension it was getting wrong. The real fix is algorithmic, not another prompt tweak: the model is far more reliable at knowing where a block of handwriting *starts* than where it ends, so `map-answers/route.ts` post-processes every region on a page (across all questions' answers and any unmatched content), sorts them top-to-bottom by start position, and deterministically stretches each one's bottom edge down to just before the next one starts. This makes full answer coverage a geometric guarantee rather than something hoped for from the model — verified on a real page where three answers are written back-to-back with almost no gap: every boundary now lands with a consistent, deliberate 0.8% seam and zero gaps or overlaps.
